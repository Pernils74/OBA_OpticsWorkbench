...
# ============================================================
# Additive placement helpers
# ============================================================


def apply_additive_offset(obj, dx, dy, dz):
    """
    Lägg till (dx,dy,dz) ovanpå befintlig Placement,
    oavsett om axis är expressionsstyrd eller ej.
    """
    pl = obj.Placement
    pe = pl.ExpressionEngine or {}
    base = pl.Base

    def wrap(expr_or_val):
        return f"({expr_or_val})"

    new_pe = dict(pe)

    # X
    if "Base.x" in pe:
        new_pe["Base.x"] = f"{wrap(pe['Base.x'])} + ({dx})"
    else:
        new_pe["Base.x"] = f"{base.x} + ({dx})"

    # Y
    if "Base.y" in pe:
        new_pe["Base.y"] = f"{wrap(pe['Base.y'])} + ({dy})"
    else:
        new_pe["Base.y"] = f"{base.y} + ({dy})"

    # Z
    if "Base.z" in pe:
        new_pe["Base.z"] = f"{wrap(pe['Base.z'])} + ({dz})"
    else:
        new_pe["Base.z"] = f"{base.z} + ({dz})"

    pl.ExpressionEngine = new_pe
